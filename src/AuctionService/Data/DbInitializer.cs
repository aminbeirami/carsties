...
public class DbInitializer
{
    ...
    {
        using var scope = app.Services.CreateScope(); // we use using so that after using any services, those services get disposed.
        SeedData(scope.ServiceProvider.GetService<AuctionDbContext>());
    }

    private static void SeedData(AuctionDbContext context)
    {
        context.Database.Migrate(); //it applies any pending migrations. and it will create the db if it does not exist

        if(context.Auctions.Any())
        {
            Console.WriteLine("Already have some data. No need to seed.");
            return;
        }
        ...
        context.AddRange(auctions); // Keeps them in memory but they are not inserted in db yet. it makes data insert-ready
        context.SaveChanges();
    }
}
